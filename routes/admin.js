...
router.get('/analytics', requireTeacher, async (req, res) => {
  try {
    const result = await getStudentFilterForTeacher(req);
    ...
    const { filter } = result;
    const students = await Student.find(filter).populate('activities');
    const totalTeams = await Team.countDocuments({});

    const studentsByClassMap = new Map();
    const studentsByYearMap = new Map();
    const activityMap = new Map();

    students.forEach((student) => {
      const className = student.className || 'غير محدد';
      const yearName = student.studentYear || 'غير محدد';
      studentsByClassMap.set(className, (studentsByClassMap.get(className) || 0) + 1);
      studentsByYearMap.set(yearName, (studentsByYearMap.get(yearName) || 0) + 1);

      (student.activities || []).forEach((activity) => {
        const name = activity.name || 'نشاط غير محدد';
    ...
    res.json({
      ...
      completedSubmissions: students.filter((s) => s.submissionComplete).length,
      paymentConfirmed: students.filter((s) => s.paymentConfirmed).length,
      canTravel: students.filter((s) => s.canTravel).length,
      totalTeams,
      studentsByClass: toArray(studentsByClassMap),
      studentsByYear: toArray(studentsByYearMap),
      topActivities: toArray(activityMap).slice(0, 10)
    });
  } catch (err) {
    console.error('Analytics error:', err);
    res.status(500).json({ error: 'فشل تحميل التقارير والتحليلات' });
  }
});
